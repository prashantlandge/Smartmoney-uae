            ...
            <div className="flex gap-3">
              <button
                onClick={async () => {
                  setValidating(true);
                  try {
                    const res = await fetch(`${API_BASE}/api/scrapers/validate`, { method: 'POST' });
                    if (res.ok) setValidationReport(await res.json());
            ...
            <div className="bg-white rounded-xl border p-6 mb-8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-xs text-gray-500">URLs Checked</p>
                  <p className="text-xl font-bold">{validationReport.summary?.urls_checked ?? 0}</p>
                </div>
                <div className={`rounded-lg p-3 ${validationReport.summary?.urls_failing > 0 ? 'bg-red-50' : 'bg-green-50'}`}>
                  <p className="text-xs text-gray-500">URLs Failing</p>
                  <p className={`text-xl font-bold ${validationReport.summary?.urls_failing > 0 ? 'text-red-700' : 'text-green-700'}`}>
                    {validationReport.summary?.urls_failing ?? 0}
                  </p>
                </div>
                <div className={`rounded-lg p-3 ${validationReport.summary?.structure_changes > 0 ? 'bg-yellow-50' : 'bg-green-50'}`}>
                  <p className="text-xs text-gray-500">Structure Changes</p>
                  <p className={`text-xl font-bold ${validationReport.summary?.structure_changes > 0 ? 'text-yellow-700' : 'text-green-700'}`}>
                    {validationReport.summary?.structure_changes ?? 0}
                  </p>
                </div>
                <div className={`rounded-lg p-3 ${validationReport.summary?.drift_alerts > 0 ? 'bg-orange-50' : 'bg-green-50'}`}>
                  <p className="text-xs text-gray-500">Drift Alerts</p>
                  <p className={`text-xl font-bold ${validationReport.summary?.drift_alerts > 0 ? 'text-orange-700' : 'text-green-700'}`}>
                    {validationReport.summary?.drift_alerts ?? 0}
                  </p>
                </div>
              </div>
              {validationReport.summary?.urls_failing > 0 && (
                <div className="mt-3">
                  <p className="text-sm font-medium text-red-700 mb-2">Failing URLs:</p>
                  <div className="space-y-1 max-h-40 overflow-y-auto">
                    {validationReport.url_health
                      ?.flatMap((p: any) =>
                        (p.urls || [])
                          .filter((u: any) => u.status !== 'ok')
                          .map((u: any) => ({ ...u, provider: p.provider }))
                      )
                      .map((u: any, i: number) => (
                        <div key={i} className="text-xs bg-red-50 rounded px-3 py-1.5 flex justify-between">
                          <span className="font-medium">{u.provider}</span>
                          <span className="text-red-600 truncate ml-2 max-w-xs">{u.url}</span>
                          <span className="text-red-500 ml-2">{u.status_code || u.status}</span>
                        </div>
                      ))}
                  </div>
                </div>
              ...
              <p className="text-xs text-gray-400 mt-3">
                Validated at {new Date(validationReport.timestamp).toLocaleString()}
              </p>
            </div>
          )}
