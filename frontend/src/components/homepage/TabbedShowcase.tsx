...
import { CreditCard, Landmark, Moon, Car, HeartPulse, Award, ChevronRight } from 'lucide-react';
    ...
    <div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 mb-6 scrollbar-hide">
        {TABS.map((tb, idx) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-button text-sm font-medium whitespace-nowrap transition-all ${
              idx === activeTab
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        {tab.products.map((product, idx) => (
          <div
            key={product.name}
            className={`bg-white rounded-card border border-surface-200 p-5 hover:shadow-elevated hover:-translate-y-0.5 transition-all duration-200 ${
              idx === 0 ? 'ring-2 ring-lime-300' : ''
            }`}
          >
            {/* Badge */}
            <div className="mb-3">
            ...
            <div className="flex items-center gap-3 mb-3">
              <ProviderLogo name={product.provider} size={36} />
              <div>
            ...
            <Link
              href={product.href}
              className="text-xs font-semibold text-brand-primary hover:text-brand-primary-700 flex items-center gap-1 transition-colors"
            >
      ...
      <div className="text-center mt-5">
        <Link
          href={tab.href}
          className="inline-flex items-center gap-1.5 text-sm font-semibold text-brand-primary hover:text-brand-primary-700 transition-colors"
        >
