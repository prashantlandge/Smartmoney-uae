...
export default function SmartRecommendQuiz({ onClose, className = '' }: Props) {
  ...
  const steps = [
    ...
    <div key="preferences">
      ...
      <div className="space-y-4">
        {/* Islamic preference */}
        <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-gray-200 hover:border-primary/30 transition-colors">
          <input
            type="checkbox"
            checked={islamicPref}
  ...
  return (
    <div className={`bg-white rounded-card border border-gray-200 shadow-elevated overflow-hidden ${className}`}>
      {/* Header */}
      <div className="bg-gradient-to-r from-primary to-primary-600 px-6 py-4 flex items-center justify-between">
        ...
        {results !== null ? (
          <div>
              ...
              <div className="space-y-3">
                {results.map((rec, idx) => (
                  <Link
                    ...
                    className="flex items-start gap-3 p-3 rounded-xl border border-gray-200 hover:border-primary/30 hover:shadow-card transition-all group"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {rec.highlight && (
                        <p className="text-xs text-primary font-medium mt-1">{rec.highlight}</p>
                      )}
                    ...
                    <ArrowRight size={14} className="text-gray-300 group-hover:text-primary shrink-0 mt-2" />
                  </Link>
                ))}
              </div>
            )}
            ...
            <button
              ...
              className="text-xs text-primary hover:text-primary-700 font-medium mt-4 transition-colors"
            >
              Start over
            </button>
          </div>
