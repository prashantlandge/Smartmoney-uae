...
logger = logging.getLogger("scrapers.pdf.rakbank")


class RAKBANKPDFScraper(PDFScraper):
    PROVIDER_ID = "b0000001-0000-0000-0000-000000000005"
    PROVIDER_NAME = "RAKBANK"
    PROVIDER_KEY = "rakbank"
    ...
    def _extract_credit_cards(self, text: str, tables: list) -> list[ScrapedProduct]:
        ...
        card_types = re.findall(
            r"(World\s*Elite|Titanium|Gold|Classic|Cashback)", text, re.I
        )
        if card_types:
            for ct in set(card_types):
                ...
                products.append(ScrapedProduct(
                    ...
                    name_en=f"RAKBANK {ct.strip()} Mastercard",
                    key_features=cf,
                ))
        elif features:
            ...
            products.append(ScrapedProduct(
                provider_id=self.PROVIDER_ID,
                category="credit_card",
                name_en="RAKBANK Credit Card (KFS)",
    ...
    def _extract_fees(self, text: str, tables: list) -> list[ScrapedProduct]:
        ...
        if features:
            ...
            return [ScrapedProduct(
                provider_id=self.PROVIDER_ID,
                category="credit_card",
                name_en="RAKBANK Schedule of Charges",
                key_features=features,
            )]
        return []
