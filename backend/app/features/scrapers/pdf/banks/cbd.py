"""CBD (Commercial Bank of Dubai) PDF parser."""
...
logger = logging.getLogger("scrapers.pdf.cbd")


class CBDPDFScraper(PDFScraper):
    ...
    def _extract_credit_cards(self, text: str, tables: list) -> list[ScrapedProduct]:
        ...
        products = []
        card_types = re.findall(r"(World|Platinum|Gold|Classic|Cashback)", text, re.I)
        if card_types:
            for ct in set(card_types):
                ...
                products.append(ScrapedProduct(
                    ...
                    name_en=f"CBD {ct.strip()} Credit Card",
                    key_features=cf,
                ))
        else:
    ...
    def _extract_fees(self, text: str, tables: list) -> list[ScrapedProduct]:
        ...
        if features:
            ...
            return [ScrapedProduct(
                provider_id=self.PROVIDER_ID,
                category="credit_card",
                name_en="CBD Schedule of Charges",
                key_features=features,
            )]
        return []
